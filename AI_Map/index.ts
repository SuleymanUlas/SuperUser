...
import { Code_Edit_Used, Job, Bootfunc, Control, Simulation, Fell, Train, Memory, Norology } from '../AI_Class/index.js';
import { SUFunc } from '../Al_Func/index.js';
 ...
 */
const nlpUsage = async (Query: string) => {
    try {
        ...
        for (let sentence of sentences) {
            ...
            const sentence_data: Record<string, any> = {
                type: 'query',
                persons: persons,
                objects: objects,
                places: places,
...
export const Map = async (Query: string, Reply: string, Status: string, User: string) => {
    const qr = { q: Query };
    const data:any = await nlpUsage(qr.q); let reply = '';
    if (data) {
        reply = await norology.Analayzing(Query.toLowerCase().replaceAll(/\s([?!\.,;:])$/g, '$1').replaceAll(/([+\-*/=%^&|<>!])/g, ' $1 '), User, data.extracted_meanings);
    }
    return reply;
}
export const Return_Fell = async (USER: string) => {

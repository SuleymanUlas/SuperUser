...
export class Bootfunc {
    ...
    async File(SearchF: string, fmime: string) {
        ...
        if (apiKey == '<=DA<apikey>TA=>' || searchEngineId == '<=DA<searchengineid>TA=>') { Bootfunc.Info('0'); return '' }
        ...
        try {
            ...
            for (const strLink of allLinks) {
                ...
                if (strLink.match(linkRegex)) {
                    strLink.replace(linkRegex, (match: string, one: string) => {
    ...
    async Weather(from: string) {
        let link = `https://www.google.com/search?q=${from} weather`;
        await page.goto(link, { waitUntil: 'domcontentloaded', timeout: 0 });
        const textValue = await page.$eval('.UQt4rd', (el: any) => el.innerText);
        if (textValue) {
            return textValue;
        }
    }
    async getSumarizeINF(SearchG: string, property: string) {
        const ceuser = new Code_Edit_Used;
        const apiKey = await ceuser.UserData('Default', { prp: 'read', param: 'apikey' }); const searchEngineId = await ceuser.UserData('Default', { prp: 'read', param: 'searchengineid' });
        if (apiKey == '<=DA<apikey>TA=>' || searchEngineId == '<=DA<searchengineid>TA=>') { Bootfunc.Info('0'); return '' }
    ...
    async GoogleDATA(SearchG: string, Element: any, Setting: string) {
        const ceuser = new Code_Edit_Used;
        const apiKey = await ceuser.UserData('Default', { prp: 'read', param: 'apikey' }); const searchEngineId = await ceuser.UserData('Default', { prp: 'read', param: 'searchengineid' });
        if (apiKey == '<=DA<apikey>TA=>' || searchEngineId == '<=DA<searchengineid>TA=>') { Bootfunc.Info('0'); return '' }
        const url = `https://www.googleapis.com/customsearch/v1?key=${apiKey}&cx=${searchEngineId}&q=${SearchG}`;
                ...
                for (let i = 0; i < linkA.length; i++) {
                    ...
                    switch (Setting) {
                        ...
                        case "text":
                            ...
                            break;
                        case "src":
                            const srcResults = await page.$$eval(Element, el => el.map((element:any) => element.src));
                            result.push(...srcResults.map(item => item));
                            break;
                        case "href":
                            const hrefResults = await page.$$eval(Element, el => el.map((element:any) => element.href));

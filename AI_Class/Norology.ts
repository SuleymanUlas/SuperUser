...
import { Bootfunc } from "./Bootfunc";
import { MatchedMessage, Memory } from "./Memory";
import { Code_Edit_Used } from "./Code_Edit_Used";
import { SUAI } from "./SUAI";
import { Fell, FellData } from "./Fell";
import { Word } from "./CreateQuery";
...
export class Norology {
     ...
     * @returns 
     */
    async Analayzing(Query: string, User: string, mapdata: string) {
        const memory = new Memory;
        let adm: string | MatchedMessage[] = await memory.Memory({ process: 'read' }, User);
        let query = Query.split(' ');
        return await this.Parse(Query, User, mapdata, adm);
    }
    async Parse(query: string, user: string, mapdata: any, memory: string | MatchedMessage[]) {
        const bot = new Bootfunc;
        let querymemory = '';
        const mems = new Memory;
        const meMory = await mems.Memory({ process: 'read' }, user);
        for (let memoryEntry of meMory) {
            querymemory += (memoryEntry as MatchedMessage).Query + " ";
        }
        ...
        const fd = new Fell;
        const feels: FellData = await fd.Data(user);
        ...
        const templates = JSON.parse((await fsp.readFile('./AllData/ReplyTemplates.json')).toString('utf-8'));
        ...
        async function generatePersonalizedReply(query: string, user: string, mapdata: any[], memory: string | MatchedMessage[]) {
            let wordsData = await loadWordData();
            let data = await ceu.UserData(user, { prp: 'all' });
            const botInfo: Record<string, number> = {};
        ...
        const USF = async (use: string, data?: any) => {
            if (use == 'weather') {
                try {
                ...
                let googleSummary: string = await bot.GoogleDATA(query, 'body', 'text') || '';
                if (!googleSummary || googleSummary.trim() === '') {
                    googleSummary = (await bot.getSumarizeINF(query, 'snippet')).toString() || '';
                }
                if (googleSummary) {
        ...
        async function getSafeString(value: any) {
        ...
        const selectWordsBasedOnQueryAndMood = async (query: string, mood: string, wordsData: Word[], data: any[], mem: string | MatchedMessage[]) => {
            let memory: string | MatchedMessage[];
            if (mem == 'Not found!') {
                memory = [
                    {
                        Query: 'how are you?',
            ...
            const filterByQuery = (words: Word[], queryArray: string[]) => {
                return words.filter((word: Word) => {
                    return queryArray.some((query: string) => {
            ...
            const previousMemory = (memory as MatchedMessage[]).find(entry => entry.Query.toLowerCase() === query.toLowerCase());
            ...
            if (previousMemory) {
                ...
                if (previousMoodFilter.adjective.length > 0) {
                    adjective = adjective.filter(word => previousMoodFilter.adjective.some((moodWord:string) => word.word.includes(moodWord)));
                }
                if (previousMoodFilter.verb.length > 0) {
                    verb = verb.filter(word => previousMoodFilter.verb.some((moodWord:string) => word.word.includes(moodWord)));
                }
        ...
        async function fixsentences(text: string) {
            if (isNaN(Number(text.charAt(0)))) {
                return (text.charAt(0).toUpperCase() + text.slice(1)).replace(/\s+/g, ' ');
            }
            return text.replace(/\s+/g, ' ');
        }
        const getReplyUntilValid = async (query: string, user: string) => {
            let reply = '';
            while (!reply.trim()) {

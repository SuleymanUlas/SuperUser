import { Code_Edit_Used, Job, Bootfunc, Control, Simulation, Fell, Train, Memory, Norology } from '../AI_Class';
import Sentiment from 'sentiment';
import { Map } from '../AI_Map';
 ...
 */
export async function SU_QUERY(DataSU:string, User:string, Query:string, Reply:string, filename:string) {
    try {
        ...
        if (Query) {
            ...
            function containsSadWords(text:string) {
...
async function handleUserData(User:string, Query:string) {
...
async function processUserInformation(Query:string, User:string) {
...
async function extractJobInfoFromQuery(Query:string) {
    const doc = nlp(Query);
...
async function extractParameterValue(Query:string, param:string, User:string) {
    const doc = nlp(Query);
    switch (param) {
        case 'age':

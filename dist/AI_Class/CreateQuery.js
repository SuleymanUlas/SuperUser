...
export class CreateQuery {
    ...
    async selectWordsBasedOnQueryAndMood(query, mood, wordsData, user) {
        ...
        let mem = await (new Memory).Memory({ process: 'read' }, user);
        let previous;
        if (mem == 'Not found!') {
            mem = [
                {
        ...
        if (queryObjects.length > 0)
            noun = filterByQuery(noun, queryObjects);
        if (queryPlaces.length > 0)
            noun = filterByQuery(noun, queryPlaces);
        if (queryPersons.length > 0)
            noun = filterByQuery(noun, queryPersons);
        if (queryTime.length > 0)
            noun = filterByQuery(noun, queryTime);
        if (queryReasons.length > 0)
            noun = filterByQuery(noun, queryReasons);
        if (queryMethods.length > 0)
            noun = filterByQuery(noun, queryMethods);
        if (queryAdjectives.length > 0)
            adjective = filterByQuery(adjective, queryAdjectives);
        if (queryVerbs.length > 0)
            verb = filterByQuery(verb, queryVerbs);
        const moodFilters = {

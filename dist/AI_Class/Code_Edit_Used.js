    ...
    async Process(status, runcode) {
        ...
        switch (status) {
            ...
            case '1':
                ...
                const cpu = await stats.cpu;
                const c = cpu.slice(0, 4);
                const ram = await stats.ram;
                const r = ram.slice(0, 4);
                const file = `${(folderSize / 1024).toFixed(2).slice(0, 4)}`;
                one = 'All Data';
                two = { c: cpu, r: ram, f: file };
                break;
            case '2':
...
Code_Edit_Used.myEmitter = new MyEmitter();
//# sourceMappingURL=Code_Edit_Used.js.map